...
Records are **positional**: adding a component to `TransferRequest` or
`TransferResponse` breaks every existing constructor call — including the test
`request(...)` helper and `TransferResponse.from`. Current component order:
`TransferRequest(idempotencyKey, sourceAccount, destinationAccount, amount)`;
`TransferResponse(id, sourceAccount, destinationAccount, amount, status, rejectionReason, createdAt)`.
