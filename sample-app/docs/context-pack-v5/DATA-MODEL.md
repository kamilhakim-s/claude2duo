...
Constructor (positional — a new field changes every call site):
`Transfer(String idempotencyKey, Account source, Account destination, BigDecimal amount, Status status, String rejectionReason)`;
`createdAt` is set inside it via `Instant.now()`.

**⚠ Two construction sites:** `TransferService.execute` builds a `Transfer` in BOTH the
REJECTED branch and the COMPLETED branch. A new Transfer field must be passed at both
call sites or it is silently null on one path.

`Account` constructor: `Account(String sortCodeAccountNumber, String currency, BigDecimal balance)`.
